...
INSERT INTO [Eleve] ([IdEleve], [Nom_Eleve], [Prenom], [Adresse], [DateNaiss], [Classe]) VALUES (1, 'Belhaj', 'Amina', 'Tunis', #2008-03-12#, '3A');

INSERT INTO [Eleve] ([IdEleve], [Nom_Eleve], [Prenom], [Adresse], [DateNaiss], [Classe]) VALUES (2, 'Karimi', 'Omar', 'Sfax', #2008-07-01#, '3A');

INSERT INTO [Eleve] ([IdEleve], [Nom_Eleve], [Prenom], [Adresse], [DateNaiss], [Classe]) VALUES (3, 'Nasri', 'Lina', 'Sousse', #2008-11-23#, '3B');
...
SELECT e.Nom_Eleve, e.Prenom, Round(Avg(n.Valeur),2) AS Moyenne
  FROM Eleve AS e INNER JOIN Note AS n ON n.IdEleve=e.IdEleve GROUP BY e.Nom_Eleve, e.Prenom;

CREATE VIEW [R2_ElevesEnEchec] AS
SELECT Nom_Eleve, Prenom FROM Eleve AS e
  WHERE (SELECT Avg(Valeur) FROM Note WHERE IdEleve=e.IdEleve) < 10;

CREATE VIEW [R3_NotesInformatique] AS
SELECT e.Nom_Eleve, n.Valeur FROM Note AS n INNER JOIN Eleve AS e ON e.IdEleve=n.IdEleve
